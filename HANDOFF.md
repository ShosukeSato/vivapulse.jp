...
- 施設は10。2026-08-18、旧「計画区画」が N-02「THE LIVING ROOM」になった。
  「まちづくり日記」は施設名ではなく、この個人記録室で準備している番組名。
  施設数を変える場合は、地図、CENTRAL、施設一覧、QA、設計文書を同時に更新する。
...
## 2-0. THE LIVING ROOM内の「まちづくり日記」を更新する

- チャンネルURLは `src/data/content.ts` の `machizukuriNikkiChannelUrl` が正。
- 2026-08-18時点では動画0本のため、館内は「開局準備中」と正直に表示している。
- 最初の動画を公開したら、実題名、公開日、尺、実サムネイルを確認してから館内へ追加する。
- 架空の予告回、波形、ON AIR、公開予定日は置かない。
